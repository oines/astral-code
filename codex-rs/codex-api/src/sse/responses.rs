...
pub fn spawn_response_stream(
    ...
    tokio::spawn(async move {
        if let Some(model) = server_model {
            let _ = tx_event
                .send(Ok(ModelStreamEvent::ServerModel(model)))
                .await;
        }
        ...
        if reasoning_included {
            let _ = tx_event
                .send(Ok(ModelStreamEvent::ServerReasoningIncluded(true)))
                .await;
        }
...
) -> std::result::Result<Option<ModelStreamEvent>, ResponsesEventError> {
    match event.kind.as_str() {
        "response.output_item.done" => {
            if let Some(item_val) = event.item {
                if let Ok(item) = serde_json::from_value::<TranscriptItem>(item_val) {
                    return Ok(Some(ModelStreamEvent::OutputItemDone(item)));
                }
        ...
        "response.output_text.delta" => {
            if let Some(delta) = event.delta {
                return Ok(Some(ModelStreamEvent::OutputTextDelta(delta)));
            }
        }
        "response.custom_tool_call_input.delta" => {
            ...
            {
                return Ok(Some(ModelStreamEvent::ToolCallInputDelta {
                    item_id,
        ...
        "response.reasoning_summary_text.delta" => {
            if let (Some(delta), Some(summary_index)) = (event.delta, event.summary_index) {
                return Ok(Some(ModelStreamEvent::ReasoningSummaryDelta {
                    delta,
                    summary_index,
                }));
            }
        ...
        "response.reasoning_text.delta" => {
            if let (Some(delta), Some(content_index)) = (event.delta, event.content_index) {
                return Ok(Some(ModelStreamEvent::ReasoningContentDelta {
                    delta,
                    content_index,
                }));
            }
        ...
        "response.created" => {
            if event.response.is_some() {
                return Ok(Some(ModelStreamEvent::Created));
            }
        }
        "response.failed" => {
        ...
        "response.output_item.added" => {
            if let Some(item_val) = event.item {
                if let Ok(item) = serde_json::from_value::<TranscriptItem>(item_val) {
                    return Ok(Some(ModelStreamEvent::OutputItemAdded(item)));
                }
        ...
        "response.reasoning_summary_part.added" => {
            if let Some(summary_index) = event.summary_index {
                return Ok(Some(ModelStreamEvent::ReasoningSummaryPartAdded {
                    summary_index,
                }));
            }
        }
...
pub async fn process_sse(
    ...
    loop {
        ...
        {
            if tx_event
                .send(Ok(ModelStreamEvent::ServerModel(model.clone())))
                .await
                .is_err()
            {
                return;
        ...
        if let Some(verifications) = model_verifications
            && tx_event
                .send(Ok(ModelStreamEvent::ModelVerifications(verifications)))
                .await
                .is_err()
        {
            return;
        ...
        if let Some(metadata) = turn_moderation_metadata
            && tx_event
                .send(Ok(ModelStreamEvent::TurnModerationMetadata(metadata)))
                .await
                .is_err()
        {
            return;
...
mod tests {
    ...
    async fn collect_events(chunks: &[&[u8]]) -> Vec<Result<ModelStreamEvent, ApiError>> {
        ...
        let (tx, mut rx) = mpsc::channel::<Result<ModelStreamEvent, ApiError>>(16);
    ...
    async fn run_sse(events: Vec<serde_json::Value>) -> Vec<ModelStreamEvent> {
        ...
        let (tx, mut rx) = mpsc::channel::<Result<ModelStreamEvent, ApiError>>(8);
    ...
    async fn parses_items_and_completed() {
        ...
        assert_matches!(
            &events[0],
            Ok(ModelStreamEvent::OutputItemDone(TranscriptItem::Message {
                role,
        ...
        assert_matches!(
            &events[1],
            Ok(ModelStreamEvent::OutputItemDone(TranscriptItem::Message { role, .. }))
                if role == "assistant"
        );
    ...
    async fn error_when_missing_completed() {
        ...
        assert_matches!(events[0], Ok(ModelStreamEvent::OutputItemDone(_)));
    ...
    async fn parses_tool_search_call_items() {
        ...
        assert_matches!(
            &events[0],
            ModelStreamEvent::OutputItemDone(TranscriptItem::ToolSearchCall {
                call_id,
                execution,
                arguments,
                ..
    ...
    async fn parses_tool_call_input_deltas() {
        ...
        assert_matches!(&events[1], ModelStreamEvent::Completed { .. });
    }
    ...
    async fn table_driven_event_kinds() {
        struct TestCase {
            ...
            expect_first: fn(&ModelStreamEvent) -> bool,
            expected_len: usize,
        }

        fn is_created(ev: &ModelStreamEvent) -> bool {
            matches!(ev, ModelStreamEvent::Created)
        }
        fn is_output(ev: &ModelStreamEvent) -> bool {
            matches!(ev, ModelStreamEvent::OutputItemDone(_))
        }
        fn is_completed(ev: &ModelStreamEvent) -> bool {
            matches!(ev, ModelStreamEvent::Completed { .. })
        }
    ...
    async fn spawn_response_stream_emits_header_events() {
        ...
        match event {
            ModelStreamEvent::ServerModel(model) => {
    ...
    async fn spawn_response_stream_ignores_model_verification_header() {
        ...
        assert!(
            !events
                .iter()
                .any(|event| matches!(event, ModelStreamEvent::ModelVerifications(_)))
        );
    }
    ...
    async fn process_sse_ignores_response_model_field_in_payload() {
        ...
        assert_eq!(events.len(), 2);
        assert_matches!(&events[0], ModelStreamEvent::Created);
        assert_matches!(
            &events[1],
    ...
    async fn process_sse_emits_server_model_from_response_headers_payload() {
        ...
        assert_matches!(
            &events[0],
            ModelStreamEvent::ServerModel(model) if model == CYBER_RESTRICTED_MODEL_FOR_TESTS
        );
        assert_matches!(&events[1], ModelStreamEvent::Created);
        assert_matches!(
            &events[2],
            ModelStreamEvent::Completed {
                response_id,
    ...
    async fn process_sse_emits_turn_moderation_metadata_field() {
        ...
        assert_matches!(
            &events[0],
            ModelStreamEvent::TurnModerationMetadata(result)
