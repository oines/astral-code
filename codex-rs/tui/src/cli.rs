...
pub struct Cli {
    ...
    // Internal controls set by the top-level `astral resume` subcommand.
    ...
    /// top-level `astral resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    /// Enable live web search for the active provider and tool surface.
