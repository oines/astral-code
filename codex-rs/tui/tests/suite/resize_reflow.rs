...
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("ASTRAL_HOME={}", codex_home.path().display()))
...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    ...
    let fallback = repo_root.join("codex-rs/target/debug/astral");
    anyhow::ensure!(
        fallback.is_file(),
        "astral binary is unavailable; run `cargo build -p codex-cli` first"
    );
    Ok(fallback)
}
