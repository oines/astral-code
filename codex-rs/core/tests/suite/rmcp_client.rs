...
async fn stdio_server_uses_configured_cwd_before_runtime_fallback() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        .with_workspace_setup(|cwd, fs| async move {
            ...
            let configured_cwd_uri = PathUri::from_abs_path(&configured_cwd);
            fs.create_directory(
                &configured_cwd_uri,
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        ...
        .with_model_info_override(text_only_model_slug, |model_info| {
...
/// This test writes to a fallback credentials file in ASTRAL_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire ASTRAL_HOME through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 3: seed an isolated ASTRAL_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _codex_home_guard = EnvVarGuard::set("ASTRAL_HOME", temp_home.path().as_os_str());
