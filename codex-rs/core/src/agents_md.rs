...
/// Loads project AGENTS.md content from every ready environment and combines it
/// with the configured global instructions.
    ...
    fn single_project_cwd(&self) -> Option<&PathUri> {
        self.entries
            ...
            .find_map(|entry| match &entry.provenance {
                ...
                InstructionProvenance::User(_) | InstructionProvenance::Internal => None,
            })
    }
}
...
impl InstructionProvenance {
    fn path(&self) -> Option<PathUri> {
        match self {
            Self::User(path) => Some(PathUri::from_abs_path(path)),
