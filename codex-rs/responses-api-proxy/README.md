...
cd path/to/astral-code/codex-rs
cargo build
echo $ASTRAL_API_KEY | ./target/debug/codex-responses-api-proxy \
    --port 60001 \
    --upstream-url https://your-provider.example/v1/responses \
    --dump-dir /tmp/proxy


# Add this to ~/.astral-code/config.toml:
...
A strict HTTP proxy that only forwards `POST` requests to `/v1/responses` to an explicitly configured upstream URL, injecting an `Authorization: Bearer <key>` header read from stdin. Everything else is rejected with `403 Forbidden`.

## Expected Usage

**IMPORTANT:** `codex-responses-api-proxy` is designed to be run by a privileged user with access to the provider API key so that an unprivileged user cannot inspect or tamper with the process. Though if `--http-shutdown` is specified, an unprivileged user _can_ make a `GET` request to `/shutdown` to shutdown the server, as an unprivileged user could not send `SIGTERM` to kill the process.

A privileged user (i.e., `root` or a user with `sudo`) who has access to the provider API key would run the following to start the server, as `codex-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv ASTRAL_API_KEY | env -u ASTRAL_API_KEY codex-responses-api-proxy \
  --http-shutdown \
  --server-info /tmp/server-info.json \
  --upstream-url https://your-provider.example/v1/responses
```

A non-privileged user would then run Astral as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
astral exec -c "model_providers.responses-proxy={ name = 'Responses Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
    -c model_provider="responses-proxy" \
    'Your prompt here'
```
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv ASTRAL_API_KEY | codex-responses-api-proxy`).
...
- Accepts exactly `POST /v1/responses` (no query string). The request body is forwarded to `--upstream-url` with `Authorization: Bearer <key>` set. All original request headers (except any incoming `Authorization`) are forwarded upstream, with `Host` overridden to the configured upstream host. For other requests, it responds with `403`.
...
```
codex-responses-api-proxy --upstream-url <URL> [--port <PORT>] [--server-info <FILE>] [--http-shutdown] [--dump-dir <DIR>]
```
...
- `--upstream-url <URL>`: Absolute URL to forward requests to. Required; there is no built-in OpenAI default.
...
- Authentication is fixed to `Authorization: Bearer <key>` to match Astral provider configuration.

For providers that require query parameters on the upstream endpoint:

```shell
printenv ASTRAL_API_KEY | env -u ASTRAL_API_KEY codex-responses-api-proxy \
  --http-shutdown \
  --server-info /tmp/server-info.json \
  --upstream-url "https://your-provider.example/v1/responses?api-version=2026-01-01"
```
...
Care is taken to restrict access/copying to the bearer token retained in memory:

- We leverage `codex_process_hardening` from this workspace so `codex-responses-api-proxy` is run with standard process-hardening techniques.
