...
#[test]
fn jina_passes_domains_and_rejects_unsupported_options() {
    ...
    assert_eq!(
        jina_url(&request).expect("Jina URL should build").as_str(),
        "https://s.jina.ai/?q=rust+async&site=example.com&site=docs.rs"
    );
    ...
    let limited_request =
        WebSearchRequest::from_input("rust async".to_string(), None, None, Some(6), today())
            .expect("request should be valid");
    assert_eq!(
        jina_url(&limited_request),
        Err(
            "Jina search does not support limit; omit limit or configure another web search provider"
                .to_string()
        )
    );
}
