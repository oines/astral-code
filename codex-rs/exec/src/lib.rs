...
enum StdinPromptBehavior {
    ...
    /// `astral exec` behavior for `astral exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `astral exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("astral_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set astral exec originator override {err:?}"
        );
    }
    ...
    let overrides = ConfigOverrides {
        ...
        personality: None,
        compact_prompt: None,
        compact_continuation_prompt: None,
    ...
    codex_core::otel_init::record_process_start(otel.as_ref(), "astral_exec");
    codex_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "astral_exec");
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "astral_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    // Print the effective configuration and initial request so users can see what Astral
    // is using.
    ...
    info!("Astral initialized with event: {session_configured:?}");
