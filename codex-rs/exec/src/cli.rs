...
#[command(
    version,
    override_usage = "astral exec [OPTIONS] [PROMPT]\n       astral exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Astral outside a Git repository.
    ...
    /// Do not load `$ASTRAL_HOME/config.toml`; auth still uses `ASTRAL_HOME`.
