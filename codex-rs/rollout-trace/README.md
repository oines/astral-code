...
> **Privacy:** Rollout tracing is not telemetry. Astral does **not** upload or
> report these traces; it writes local bundles only when
> `ASTRAL_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during an Astral session. It records raw runtime evidence into a local bundle on
...
Hot-path Astral code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Astral got
there.
...
one from `ASTRAL_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Astral session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `astral debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Astral rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
astral debug trace-reduce <trace-bundle>
```
