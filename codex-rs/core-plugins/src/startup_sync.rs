            ...
            match sync_openai_plugins_repo_via_http(codex_home, api_base_url) {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(codex_home) {
                        warn!(
                            error = %http_err,
                            "GitHub HTTP sync failed for curated plugin sync; keeping existing local curated plugins snapshot"
                        );
                    }
                    Err(format!(
                        "git sync failed for curated plugin sync: {err}; GitHub HTTP sync failed for curated plugin sync: {http_err}"
                    ))
                }
            }
        }
