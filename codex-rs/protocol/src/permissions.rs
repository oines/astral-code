...
const PROTECTED_METADATA_CODEX_PATH_NAME: &str = ".astral-code";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.astral-code -> <root>/decoy`, bwrap must still see
            // `<root>/.astral-code`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.astral-code` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .astral-code read-only to the agent by
...
mod tests {
    ...
    fn writable_roots_proactively_protect_missing_dot_codex() {
        ...
        let expected_dot_codex = expected_root.join(".astral-code");
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".astral-code".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    fn writable_roots_skip_default_dot_codex_when_explicit_user_rule_exists() {
        ...
        .expect("absolute canonical root");
        let explicit_dot_codex = expected_root.join(".astral-code");
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".astral-code".to_string()),
            "explicit .astral-code rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_codex),
            "explicit .astral-code rule should win over the default protected carveout"
        );
        assert!(
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_codex_config = cwd.path().join(".astral-code").join("config.toml");
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".astral-code/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let codex_dir = real_root.join(".astral-code");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&codex_dir).expect("create .astral-code");
        ...
        let expected_codex = link_root.join(".astral-code");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_codex = expected_root.join(".astral-code");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let dot_codex = root.join(".astral-code");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_codex).expect("create .astral-code symlink");
        ...
        .expect("absolute .astral-code symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let codex_dir = real_tmpdir.join(".astral-code");
