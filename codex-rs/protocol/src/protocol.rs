...
/// privileges of the agent (e.g. `.astral-code`, `.git`, notably `.git/hooks`) under
...
mod tests {
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".astral-code")).expect("create .astral-code");
        let canonical_cwd = codex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_codex =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".astral-code"))
                .expect("canonical .astral-code");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
