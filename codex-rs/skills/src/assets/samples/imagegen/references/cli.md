...
Set a stable path to the skill CLI (default `ASTRAL_HOME` is `~/.astral-code`):

```
export ASTRAL_HOME="${ASTRAL_HOME:-$HOME/.astral-code}"
export IMAGE_GEN="$ASTRAL_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$ASTRAL_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
