...
impl SurfaceState {
    ...
    pub(crate) fn toggle_block_viewer_raw(&mut self) -> bool {
        let Some(entry_id) = self.block_viewer_entry_id().map(str::to_string) else {
            return false;
        };
    ...
    pub(crate) fn block_viewer_copy_text(&mut self) -> Option<String> {
        let entry_id = self.block_viewer_entry_id()?.to_string();
    ...
    pub(super) fn block_viewer_text_mode(&self) -> BlockTextMode {
        self.block_viewer_entry_id()
            .filter(|entry_id| self.scrollback.is_raw_entry(entry_id))
