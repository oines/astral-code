...
fn mcp_tool_action_form_preserves_parameters_and_persist_choice() {
    let request = mcp_action_request(
        ...
        serde_json::json!({
            ...
            "tool_params_display": [
                {"name": "title", "display_name": "Title", "value": "Roadmap review"},
...
#[test]
fn tool_suggestion_preserves_browser_confirmation_and_persistent_decline() {
    let request = || {
        mcp_action_request(
            15,
            ...
            serde_json::json!({
                ...
                "install_url": "https://apps.example/calendar"
            }),
        )
    };
    let mut pending = PendingInteractions::new("thread-1");
    ...
    assert_eq!(
        host.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)),
        PromptInteractionOutcome::OpenExternalUrl {
            url: "https://apps.example/calendar".to_string()
        }
    );
    assert!(matches!(
        host.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)),
        PromptInteractionOutcome::Submit(submission)
            if submission.result == serde_json::json!({
                "action": "accept", "content": null, "_meta": null
            })
    ));
...
#[test]
fn malformed_tool_suggestion_falls_back_to_answerable_generic_action() {
    let mut pending = PendingInteractions::new("thread-1");
    pending.observe_request(mcp_action_request(
        16,
        "Allow this MCP request?",
        serde_json::json!({"codex_approval_kind": "tool_suggestion"}),
    ));
    let mut host = PromptInteractionHost::new();
    assert!(host.sync(&pending));
    assert!(host.is_presentable());
