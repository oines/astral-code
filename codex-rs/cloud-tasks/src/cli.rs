...
pub enum Command {
    /// Submit a new Astral Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of an Astral Cloud task.
    Status(StatusCommand),
    /// List Astral Cloud tasks.
    List(ListCommand),
    /// Apply the diff for an Astral Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for an Astral Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Astral Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `astral cloud` to browse).
    ...
    /// Git branch to run in Astral Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Astral Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Astral Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Astral Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
