//! Implements the `astral doctor` diagnostic report.
//!
...
///
/// This is the CLI entry point for astral doctor. It does not repair issues;
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun astral doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
...
/// JSON support report emitted by `astral doctor --json`.
///
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    if path_entries.len() > 1 {
        details.push(format!("PATH astral entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH astral #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary = "npm install -g astral-code would update a different install".to_string();
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("astral-code");
...
fn codex_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["astral"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "astral"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("ASTRAL_HOME: {}", config.codex_home.display()));
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "ASTRAL_HOME", &config.codex_home);
    ...
    if status == CheckStatus::Fail {
        check = check.remediation(
            "Back up ASTRAL_HOME, then remove or repair the affected SQLite database.",
        );
    }
    check
}
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "ASTRAL_HOME was resolved without config",
        )
        .detail(format!("ASTRAL_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "ASTRAL_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/astral-code");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("astral-code")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/astral-code");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("astral-code"),
            }
        );
    }
