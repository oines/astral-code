...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_recovery_followup_success = tracing::field::debug(&snapshot.auth_recovery_followup_success),
        auth_recovery_followup_status = tracing::field::debug(&snapshot.auth_recovery_followup_status),
        auth_env_astral_api_key_present = tracing::field::debug(auth_env.astral_api_key_env_present),
        auth_env_astral_api_key_enabled = tracing::field::debug(auth_env.astral_api_key_env_enabled),
...
impl FeedbackSnapshot {
    ...
    /// Feedback uploads are intentionally disabled in astral-code.
    pub fn upload_feedback(&self, _options: FeedbackUploadOptions<'_>) -> Result<()> {
        anyhow::bail!("feedback upload is disabled in astral-code")
    }
