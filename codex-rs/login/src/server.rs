...
use crate::auth::load_codex_oauth_auth;
use crate::auth::revoke_superseded_codex_oauth_auth;
use crate::auth::save_codex_oauth_auth;
use crate::default_client::codex_oauth_originator;
...
pub(crate) async fn persist_tokens_async(
    ...
    let (previous_auth, auth) = tokio::task::spawn_blocking(move || {
        let previous_auth = match load_codex_oauth_auth(&codex_home, auth_credentials_store_mode) {
            Ok(auth) => auth,
            Err(err) => {
        ...
        save_codex_oauth_auth(&codex_home, &auth, auth_credentials_store_mode)?;
        Ok::<_, io::Error>((previous_auth, auth))
    })
    .await
    .map_err(|e| io::Error::other(format!("persist task failed: {e}")))??;

    revoke_superseded_codex_oauth_auth(previous_auth.as_ref(), &auth).await;
...
mod tests {
    ...
    async fn persist_tokens_async_revokes_previous_auth_without_failing_login() -> anyhow::Result<()>
    {
        ...
        let auth = load_codex_oauth_auth(codex_home.path(), AuthCredentialsStoreMode::File)?
