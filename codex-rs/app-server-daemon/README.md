# astral app-server daemon

> `astral app-server daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`astral app-server daemon` backs the machine-readable `astral app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Astral instances launched over SSH, including fresh developer
...
```sh
astral app-server daemon start
astral app-server daemon restart
astral app-server daemon enable-remote-control
astral app-server daemon disable-remote-control
astral app-server daemon stop
astral app-server daemon version
astral app-server daemon bootstrap --remote-control
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/oines/astral-code/main/install.sh | sh
$HOME/.astral-code/packages/standalone/current/astral app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `ASTRAL_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Astral is installed through `install.sh` and always launches
the standalone managed binary under `ASTRAL_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `ASTRAL_HOME/packages/standalone/current/astral` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `ASTRAL_HOME/packages/standalone/current/astral` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `astral remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `ASTRAL_HOME`, so a concurrent
...
The daemon stores its local state under `ASTRAL_HOME/app-server-daemon/`:
