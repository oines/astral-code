...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(codex_home) = std::env::var("ASTRAL_HOME") {
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_astral_app_runtime_bin_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.astral-code`, and `.agents`.
        //
...
mod tests {
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let codex_home = temp.path().join("codex-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".astral-code");
