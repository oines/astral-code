#!/usr/bin/env python3
"""Stage and optionally package the astral-code npm module."""
...
CODEX_NPM_NAME = "astral-code"
CODEX_PACKAGE_COMPONENT = "astral-package"

# `npm_name` is the local optional-dependency alias consumed by `bin/astral.js`.
# The underlying package published to npm is always `astral-code`.
CODEX_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "astral-code-linux-x64": {
        "npm_name": "astral-code-linux-x64",
        "npm_tag": "linux-x64",
        "target_triple": "x86_64-unknown-linux-musl",
        "os": "linux",
        "cpu": "x64",
    },
    "astral-code-linux-arm64": {
        "npm_name": "astral-code-linux-arm64",
        "npm_tag": "linux-arm64",
        "target_triple": "aarch64-unknown-linux-musl",
        "os": "linux",
        "cpu": "arm64",
    },
    "astral-code-darwin-x64": {
        "npm_name": "astral-code-darwin-x64",
        "npm_tag": "darwin-x64",
        "target_triple": "x86_64-apple-darwin",
        "os": "darwin",
        "cpu": "x64",
    },
    "astral-code-darwin-arm64": {
        "npm_name": "astral-code-darwin-arm64",
        "npm_tag": "darwin-arm64",
        "target_triple": "aarch64-apple-darwin",
        "os": "darwin",
        "cpu": "arm64",
    },
    "astral-code-win32-x64": {
        "npm_name": "astral-code-win32-x64",
        "npm_tag": "win32-x64",
        "target_triple": "x86_64-pc-windows-msvc",
        "os": "win32",
        "cpu": "x64",
    },
    "astral-code-win32-arm64": {
        "npm_name": "astral-code-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "astral-code": ["astral-code", *CODEX_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "astral-code": [],
    "astral-code-linux-x64": [CODEX_PACKAGE_COMPONENT],
    "astral-code-linux-arm64": [CODEX_PACKAGE_COMPONENT],
    "astral-code-darwin-x64": [CODEX_PACKAGE_COMPONENT],
    "astral-code-darwin-arm64": [CODEX_PACKAGE_COMPONENT],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the Astral CLI npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="astral-code",
        help="Which npm package to stage (default: astral-code).",
    )
    parser.add_argument(
        "--version",
...
def main() -> int:
    ...
    try:
        ...
        if release_version:
            ...
            if package == "astral-code":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/astral.js --version\n"
                    f"    node {staging_dir_str}/bin/astral.js --help\n\n"
                )
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="astral-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "astral-code":
        ...
        shutil.copy2(CODEX_CLI_ROOT / "bin" / "astral.js", bin_dir / "astral.js")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="astral-npm-pack-") as pack_dir_str:
